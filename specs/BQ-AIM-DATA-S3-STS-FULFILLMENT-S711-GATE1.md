...
- `Listing.raw_metadata` JSON extension for S3 fulfillment listings. Required shape: `{ "connection_id": "<vectorAIz S3Connection id>", "vectoraiz_instance_url": "https://...", "object_key": "path/to/object", "bucket": "seller-bucket", "region": "us-east-1" }`. `role_arn` and `external_id` are forbidden on marketplace-side listing/order state and remain only in the seller's vectorAIz `S3Connection`.
...
Walkthrough conversation flow: (1) Generate per-seller ExternalId following the locked format `aim-data-seller-{uuid}-{32hex}`. Persist to a pre-row `S3Connection` with status=onboarding in the seller's vectorAIz instance. (2) Generate trust + permissions policy JSON with seller's bucket name. Display via allAI chat with copy-to-clipboard buttons. (3) Wait for seller to paste role ARN into the vectorAIz-backed onboarding form. (4) Invoke `sts_probe` through vectorAIz against the pasted ARN. (5) On success: finalize the vectorAIz S3Connection row, publish marketplace listing metadata to `Listing.raw_metadata`, then transition to bucket browser. (6) On AccessDenied: `diagnostics.classify_assume_role_error` produces structured remediation (e.g., "Your role's trust policy is missing the ExternalId condition. Add this StringEquals block: ..."). Surface to seller. (7) Idempotent flow with checkpoint state — seller can walk away and resume from the pre-row.
...
- `Listing.raw_metadata` JSON extension for S3 fulfillment listings: `{connection_id, vectoraiz_instance_url, object_key, bucket, region}`. These fields are the complete marketplace-side carrier for C6. `role_arn` and `external_id` MUST NOT appear in `Listing.raw_metadata`, order snapshots, token payloads, logs, or platform DB tables.
...
Scope: end-to-end with moto STS+S3 fixtures. Files: `vectoraiz/tests/integration/test_s3_sts_scan_end_to_end.py`, `vectoraiz/tests/integration/test_s3_sts_sign_url.py`, `vectoraiz/tests/integration/test_s3_sts_scan_pagination_resume_across_multiple_pages.py`, `ai-market-backend/tests/integration/test_s3_sts_purchase_flow.py`, `ai-market-backend/tests/integration/test_aim_data_onboarding_agent.py`, `ai-market-backend/tests/integration/test_aim_data_onboarding_resume_from_abandoned_pre_row_s3connection.py`. Deps: C5 + C6 + C7. Parallel: no. LoC: ~250. ACs: (a) Full scan-to-sign-to-buyer-fetch flow passes against moto. (b) Walkthrough agent unit-tested for all error classification paths. (c) STS session expiry simulated; refresh path verified. (d) ExternalId mismatch surfaces correct diagnostic. (e) Abandoned onboarding resumes from pre-row with ExternalId preserved. (f) Large-bucket scan resumes across 3+ pages without duplicate object metadata.

## §5 Test plan

**Unit tests:**

- `tests/unit/test_sts_credential_broker.py`: mocked boto3 client. Verify cache key tuple `(role_arn, region, session_purpose)`, 2-min refresh-buffer logic, RoleSessionName format compliance with locked decision, and distinct clients for `order-{order_id}` versus `agent-qa-{listing_id}`.
